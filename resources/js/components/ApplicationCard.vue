<template>
    <div
      class="application-card bg-white shadow-lg rounded-lg p-6 mb-6 hover:shadow-xl transition-shadow cursor-pointer"
      @click="handleClick"
    >
      <div class="flex items-center justify-between">
        ...
        <div v-if="statusBadge" :class="statusBadgeClass" class="px-3 py-1 rounded-full text-sm font-medium">
          {{ statusBadge }}
        </div>
      </div>
      ...
      <div v-if="showDownloadButtons" class="mt-4 flex space-x-4">
        <a
          :href="application.cv_url || `/storage/${application.cv}`"
          target="_blank"
          rel="noopener noreferrer"
          class="flex items-center bg-blue-500 text-white px-3 py-2 rounded hover:bg-blue-600 transition-colors"
          @click.stop
        >
        ...
        <a
          :href="application.cover_letter_url || `/storage/${application.cover_letter}`"
          target="_blank"
          rel="noopener noreferrer"
          class="flex items-center bg-blue-500 text-white px-3 py-2 rounded hover:bg-blue-600 transition-colors"
          @click.stop
      ...
      <div class="mt-6 flex items-center space-x-4">
        ...
        <button
          ...
          class="flex items-center text-red-500 hover:text-red-600 font-medium transition-colors"
        >
        ...
        <div v-else-if="isCompany" class="flex space-x-4">
          <button
            ...
            class="flex items-center bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <button
            ...
            class="flex items-center bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
