<template>
    <div class="p-6 sm:p-10 text-2xl font-bold md:pl-32">
      <div v-if="jobOffers.length">
        <h3>Offres d'emploi en attente de validation</h3>
        <div class="grid lg:grid-cols-2 xl:grid-cols-3 gap-4 mt-10">
          ...
          >
            ...
            <div class="flex justify-between px-4 py-2">
              <button
                @click.stop="validateJobOffer(job.id)"
                class="bg-green-500 text-white px-3 py-2 rounded hover:bg-green-600"
              >
                Valider
              </button>
              <button
                @click.stop="deleteJobOffer(job.id)"
        ...
        <!-- Pagination -->
        <div
          v-if="jobOffersPagination.total > jobOffersPagination.per_page"
          class="mt-4 flex justify-between"
        >
          <button
            @click="fetchPendingJobOffers(jobOffersPagination.current_page - 1)"
            :disabled="!jobOffersPagination.prev_page_url"
          ...
          <button
            @click="fetchPendingJobOffers(jobOffersPagination.current_page + 1)"
            :disabled="!jobOffersPagination.next_page_url"
            class="bg-gray-200 px-4 py-2 rounded disabled:opacity-50"
          >
            Suivant
          </button>
