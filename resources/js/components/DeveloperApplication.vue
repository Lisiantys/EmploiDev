...
<div class="p-6 sm:p-20 font-bold md:pl-32">    
  <h2 class="text-2xl font-bold mb-4">Mes Candidatures</h2>
    ...
    <div v-if="acceptedApplications.length > 0" class="mb-8">
      ...
      <div v-for="application in acceptedApplications" :key="application.id" @click="openModal(application.id)" class="border-2 border-green-500 shadow-lg rounded-lg p-4 mb-4 cursor-pointer">
    ...
    <div v-if="pendingApplications.length > 0" class="mb-8">
      ...
      <div v-for="application in pendingApplications" :key="application.id" @click="openModal(application.id)" class="border-2 border-blue-400 shadow-lg rounded-lg p-4 mb-4 cursor-pointer">
        <p><strong>Offre d'emploi :</strong> {{ application.job_offer.name }}</p>
        <p><strong>Entreprise :</strong> {{ application.job_offer.company.name }}</p>
        <p><strong>Lieu :</strong> {{ application.job_offer.location.city }}, {{ application.job_offer.location.postal_code }}</p>
        <p><strong>Description :</strong> {{ application.description }}</p>
    ...
    <div v-if="rejectedApplications.length > 0" class="mb-8">
      ...
      <div v-for="application in rejectedApplications" :key="application.id" @click="openModal(application.id)" class="border-2 border-red-500 shadow-lg rounded-lg p-4 mb-4 cursor-pointer">
        <p><strong>Offre d'emploi :</strong> {{ application.job_offer.name }}</p>
        <p><strong>Entreprise :</strong> {{ application.job_offer.company.name }}</p>
        <p><strong>Lieu :</strong> {{ application.job_offer.location.city }}, {{ application.job_offer.location.postal_code }}</p>
        <p><strong>Description :</strong> {{ application.description }}</p>
