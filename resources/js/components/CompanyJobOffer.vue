  ...
  <div class="p-6 sm:p-20 font-bold md:pl-32">
    ...
    <div v-if="validatedJobOffers.length > 0" class="mb-8">
      ...
      <div v-for="offer in validatedJobOffers" :key="offer.id"
        class="border-2 border-green-500 shadow-lg rounded-lg p-4 mb-4 cursor-pointer" @click="viewApplications(offer)">
        ...
        <!-- Bouton Supprimer -->
        <button @click.stop="deleteJobOffer(offer.id)"
    ...
    <div v-if="nonValidatedJobOffers.length > 0" class="mb-8">
      ...
      <div v-for="offer in nonValidatedJobOffers" :key="offer.id"
        ...
        <p><strong>Ville :</strong> {{ offer.location.city }}</p>
        <!-- Bouton Supprimer -->
        <button @click="deleteJobOffer(offer.id)" class="mt-4 bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">
          Supprimer
        </button>
      </div>
    </div>
