<template>
    <div class="hidden flex-col md:flex lg:flex-row mt-10 p-6 border-4 border-blue-400 rounded-lg shadow-md">
        ...
        <div class="flex flex-col justify-between w-5/6">
            <div class="mb-4">
                ...
                <div class="flex justify-between w-96 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
        <div v-for="contract in contracts" :key="contract.id" class="flex items-center mb-2">
            <input :id="`contract-${contract.id}`" type="checkbox" class="form-checkbox h-4 w-4 text-blue-600" 
                   v-model="selectedContracts" :value="contract.id" />
            <label :for="`contract-${contract.id}`" class="ml-2 block text-sm font-semibold text-gray-800">
                {{ contract.name }}
            </label>
        </div>
    </div>
            ...
            <div class="mb-4">
                ...
                <div class="flex justify-between pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
        <div v-for="year in yearsExperiences" :key="year.id" class="flex items-center mb-2">
            ...
            <div class="w-96">
                ...
                <select v-model="selectedLocalisation" id="localisation" name="localisation" class="mt-1 block w-full pl-3 pr-10 py-2 border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                    <option value="" disabled selected>Sélectionnez une localisation</option>
                    <option v-for="localisation in localisations" :key="localisation.id" :value="localisation.id">{{ localisation.city }}, {{ localisation.postal_code }}</option>
                    <!-- Vous pouvez ajouter une option pour entrer manuellement le code postal si nécessaire -->
                </select>
            </div>
        </div>
...
const searchDevelopers = async () => {
    try {
        ...
        const response = await Axios.post("/api/developers/filter", {
            contract_id: selectedContracts.value,
            year_id: selectedYearsExperiences.value,
            type_id: selectedDeveloperType.value,
            programming_languages: selectedProgrammingLanguages.value,
        ...
        console.error("Erreur lors de la recherche :", error.response ? error.response.data : error);
    }
};
...
const resetFilters = () => {
    ...
    selectedProgrammingLanguages.value = []; // Réinitialiser les langages de programmation
...
const fetchYearsExperiences = async () => {
    try {
        ...
        yearsExperiences.value = response.data; // Stocke les années d'expérience récupérées
    } catch (error) {
...
onMounted(() => {
    ...
    fetchLocations();
    fetchYearsExperiences(); // Appel de la nouvelle fonction ici
});
</script>
