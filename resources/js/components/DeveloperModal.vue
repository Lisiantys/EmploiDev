      ...
      <div v-if="developer && developer.first_name">
        ...
        <div class="p-6">
          ...
          <div class="flex flex-col md:flex-row">
            <!-- Profile Image -->
            <div class="md:w-1/3">
              <img :src="profileImage" alt="Profile Image" class="w-full h-auto rounded-lg object-cover">
            </div>
            <!-- Developer Information -->
            <div class="md:w-2/3 md:pl-6 mt-4 md:mt-0">
              <p><strong>Nom:</strong> {{ developer.first_name }} {{ developer.surname }}</p>
              <p v-if="developer.types_contract"><strong>Type de Contrat:</strong> {{ developer.types_contract.name }}
              </p>
              <p v-if="developer.types_developer"><strong>Type de Développeur:</strong> {{
                developer.types_developer.name }}</p>
              <p v-if="developer.years_experience"><strong>Années d'Expérience:</strong> {{
                developer.years_experience.name }}</p>
              <p v-if="developer.location"><strong>Ville:</strong> {{ developer.location.city }}, {{
